...
program
    .description('Create a SWIFT app in current directory!')
    .option('-t, --target [name]', 'Initialize the project with a variant [swift-boilerplate]', 'swift-boilerplate')
    .parse(process.argv);
...
if (!selectedVariant) {
    console.log(chalk.red.bold(`${program.variant} is not a valid SWIFT Boilerplate variant. Execute 'swift-cli list' to list available variants`));
    exit(1);
}
